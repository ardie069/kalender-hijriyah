...
export function GlobalSettings({
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-12">
      {/* Metode Hijriyah — hanya tampil saat mode hijri */}
      {dateSystem === "hijri" && (
        <div className="md:col-span-8 bg-white/40 dark:bg-card-dark/40 backdrop-blur-xl p-2 rounded-3xl border border-white/40 dark:border-white/5 flex flex-wrap gap-2 animate-in fade-in slide-in-from-top-4 duration-500">
          {METHODS.map((m) => (
            <button
              key={m.id}
      ...
      <div
        className={`${dateSystem === "hijri" ? "md:col-span-4" : "md:col-span-12"} flex bg-gray-200/50 dark:bg-white/5 p-2 rounded-3xl border border-gray-100 dark:border-white/5`}
      >
        <button
