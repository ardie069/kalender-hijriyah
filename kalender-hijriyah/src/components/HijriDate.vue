<template>
    ...
    <div v-if="hijriEndPrediction" :class="[hijriEndPredictionClass, 'mt-4 p-4 rounded-lg', darkMode ? 'bg-gray-700' : 'bg-gray-200']">
        ...
        try {
          ...
          if (dateData?.hijriDate) {
            ...
            if (endMonthData?.estimatedEndOfMonth) {
              const { hijri: estimatedHijri, moonAltitude, elongation, moonAge, explanation } = endMonthData.estimatedEndOfMonth;
              ...
              const now = new Date();
              let predictionContent = `<p class="text-gray-500">🔍 Prediksi hilal hanya tersedia saat tanggal 29 Hijriyah.</p>`;
