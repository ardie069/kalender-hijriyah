...
def get_hijri_date(lat, lon, method="global", timezone="UTC"):
    ...
    if hijri_date["day"] == 29:
        conj_jd = get_conjunction_time(effective_jd - 1)
        is_new_month = evaluate_new_month(
            method, lat, lon, conj_jd, sunset_utc, now_local
        )
...
def evaluate_new_month(method, lat, lon, conj_jd, sunset_utc, now_local):
    """
    ...
    if method == "global":
        ...
        sunset_mekkah = get_sunset_time(
            now_local.date(), mekkah_lat, mekkah_lon, mekkah_zone
        )
        if sunset_mekkah is None:
            return False
        jd_mekkah_sunset = jd_from_datetime(sunset_mekkah.astimezone(pytz.utc))
        return conj_jd < jd_mekkah_sunset

    elif method == "hisab":
        jd_sunset = jd_from_datetime(sunset_utc)
