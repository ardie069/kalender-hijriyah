...
func (em *EphemerisManager) GetTimeByAltitude(date time.Time, lat, lon, targetAlt float64, rising bool) (time.Time, error) {
	...
	if rising {
		// Cari dari 10 jam sebelum noon sampe noon+2 (Subuh/Terbit)
		// Window harus cukup lebar agar tidak terpotong di bujur timur
		start = noon.Add(-10 * time.Hour)
		end = noon.Add(2 * time.Hour)
	} else {
		// Cari dari noon-2 sampe 10 jam setelah noon (Maghrib/Isya)
...
func (em *EphemerisManager) GetAsrTimeWithFactor(dhuhr time.Time, lat, lon float64, shadowFactor int) time.Time {
